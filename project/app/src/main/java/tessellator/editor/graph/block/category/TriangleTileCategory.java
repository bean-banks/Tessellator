...
/**
 * An block category object which creates and draws triangle tile blocks.
 * Note that a hexagon tile is a root block, so when it is created it is
 * added straight to the editor's canvas.
 */
public class TriangleTileCategory extends BlockCategory {

    public TriangleTileCategory(BlockTheme theme) {
		super(theme, "Triangle Tile");
	}
	...
	public void draw(Block block) {
        ...
        // Define the center coordinates of the triangle
        Point2D center = new Point2D(canvasWidth/2, canvasHeight/2);

        // Set the radius length from the center to a vertex for the triangle
        double radius = (canvasWidth-30)/2;

        // Starting angle to position one vertex above the center
        double startAngleRad = Math.toRadians(150);

		// Calculate the coordinates of the vertices
		List<Point2D> vertices = new ArrayList<>();
		for (int i = 0; i < 3; i++) {
            double angleRad = startAngleRad + Math.toRadians(120 * i); // Angle in radians
		...
		Drawing drawing = new Drawing(xcoords, ycoords, labels, color, Category.TRIANGLE_TILE, block);
