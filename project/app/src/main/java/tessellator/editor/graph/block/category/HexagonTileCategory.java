...
/**
 * An block category object which creates and draws hexagon tile blocks.
 * Note that a hexagon tile is a root block, so when it is created it is
 * added straight to the editor's canvas.
 */
public class HexagonTileCategory extends BlockCategory {
	
    public HexagonTileCategory(BlockTheme theme) {
		super(theme, "Hexagon Tile");
	}
	...
	public Optional<Block> createBlock() {
		...
		// root blocks a drawn as soon as they are created because they are satisfied
		draw(block);
	...
	public void draw(Block block) {
        ...
        // Define the center coordinates of the hexagon
        Point2D center = new Point2D(canvasWidth/2, canvasHeight/2);

        // Set the radius length from the center to a vertex for the hexagon
        double radius = (canvasWidth-30)/2;

        // Starting angle to position one vertex above the center
        double startAngleRad = Math.toRadians(120);

		// Calculate the coordinates of the vertices
		List<Point2D> vertices = new ArrayList<>();
		for (int i = 0; i < 6; i++) {
            double angleRad = startAngleRad + Math.toRadians(60 * i); // Angle in radians
		...
		Drawing drawing = new Drawing(xcoords, ycoords, labels, color, Category.HEXAGON_TILE, block);
